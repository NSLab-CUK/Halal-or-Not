...
def train(args):
    ...
    ft_loss_list, ft_time_training = fine_tuning_train(model, data, fine_tuning_optimizer, device, args)

    logging.info("FINALLY -------")
 

def fine_tuning_train(model, data, optimizer, device, args):
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        if (epoch % args.evaluate_every) == 0 or epoch == args.n_epoch:
            ...
            best_recall, should_stop = early_stopping(metrics_list['f1'], args.stopping_steps)

            if should_stop:
                break

            if metrics_list['f1'].index(best_recall) == len(epoch_list) - 1:
                embeddings = model.gat_embeddings()
    ...
    evaluation(args, model, data, device,time1)

    return ft_loss_list, ft_time_training
 

def evaluate_score(model, head_ids, tail_ids, labels, device):
    model.eval()
...
def evaluation(args, model, data, device,time1):
