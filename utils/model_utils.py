...
def save_model(model, model_dir, current_epoch, last_best_epoch=None, name="training"):
    ...
    model_state_file = os.path.join(model_dir, '{}_model_epoch{}.pt'.format(name, current_epoch))
    


    print(f"model_state_file: {model_state_file}")
    torch.save({'model_state_dict': model.state_dict(),'epoch': current_epoch}, model_state_file)


    if last_best_epoch is not None and current_epoch != last_best_epoch:
        old_model_state_file = os.path.join(
            model_dir, '{}_model_epoch{}.pt'.format(name, last_best_epoch))
