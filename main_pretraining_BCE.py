...
def train(args):
    ...
    model = LiteralKG(args, data.n_entities, data.n_relations, data.A_in,  data.num_embedding_table, data.text_embedding_table)
    print(f" model.entity_embed: { model.entity_embed.weight.shape}")
    ...
    pt_loss_list, pt_time_training, best_epoch = pre_training_train(model, data, pre_training_optimizer, device, args, writer)
    ...
    print(f"# best_epoch for fine-tuning step: {best_epoch}")
...
def pre_training_train(model, data, optimizer, device, args, writer):
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        n_data_samples = int(len(list(data.train_kg_dict)) * args.epoch_data_rate)  # h: (t,r)
        ...
        for iter in tqdm(range(1, n_kg_batch + 1), desc=f"EP:{epoch}_train"):
            ...
            time1 = time()
            kg_batch_head, kg_batch_relation, kg_batch_pos_tail, kg_batch_neg_tail = data.generate_kg_batch(epoch_sampling_data_dict, 
                                                                                                            data.pre_training_batch_size, 
                                                                                                            list(data.training_tails))
            ...
            kg_batch_loss = model(kg_batch_head, kg_batch_relation, kg_batch_pos_tail, kg_batch_neg_tail, device=device, mode='pre_training')

            if np.isnan(kg_batch_loss.cpu().detach().numpy()):
                logging.info('ERROR (Pre-training): Epoch {:04d} Iter {:04d} / {:04d} Loss is nan.'.format(epoch, iter,n_kg_batch))
                sys.exit()
        ...
        logging.info('Update Attention: Epoch {:04d} | Total Time {:.1f}s'.format(epoch, time() - time2))
    ...
    return pt_loss_list, pt_time_training, best_epoch
